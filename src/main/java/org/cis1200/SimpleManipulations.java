...
public class SimpleManipulations {
    ...
    public static PixelPicture rotateCCW(PixelPicture pic) {
        ...
        //iterate backwards through the cols
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                tgt[(w - 1) - col][row] = src[row][col];
            }
        }
    ...
    //function to print all the elements in 1d array
    public static void printPixelArr(Pixel[] arr) {
        System.out.print("{");
        for (Pixel el: arr) {
            System.out.print(el + " ");
        }
    ...
    public static void print2dPixelArr(Pixel[][] arr) {
        for (Pixel[] array: arr) {
            printPixelArr(array);
        }
    }
    ...
    public static PixelPicture border(
        ...
        Pixel[][] src = pic.getBitmap();
        Pixel[][] result = new Pixel[pic.getHeight() + 2 * borderWidth][pic.getWidth() + 2 * borderWidth];


        //filling the entire m2d array with the border color
        for (int rowIndex = 0; rowIndex < result.length; rowIndex++) {
            for (int colIndex = 0; colIndex < result[0].length; colIndex++) {
                result[rowIndex][colIndex] = borderColor;
            }
        }

        //copying the org image in into the result array
        for (int row = borderWidth; row < src.length + borderWidth; row++) {
            System.arraycopy(src[row - borderWidth], 0, result[row], borderWidth, src[0].length);
        }
    ...
    public static void compare2dArray(Pixel[][] arr1, Pixel[][] arr2) {
        for (int row = 0; row < arr1.length; row++) {
            for (int col = 0; col < arr1[0].length; col++) {
                if (!(arr1[row][col].equals(arr2[row][col]))) {
                    System.out.println("row: " + row + "col: " + col + " " + arr1[row][col] + " != " + arr2[row][col]);
                }
            }
        }
    }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int rowIndex = 0; rowIndex < result.length; rowIndex++) {
            for (int colIndex = 0; colIndex < result[0].length; colIndex++) {
                ...
                result[rowIndex][colIndex] = new Pixel(avg, avg, avg);
            }
        }
        return new PixelPicture(result);
    }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int rowIndex = 0; rowIndex < result.length; rowIndex++) {
            for (int colIndex = 0; colIndex < result[0].length; colIndex++) {
                ...
                int newR = (int) Math.round(r * rFactor);
                int newG = (int) Math.round(g * gFactor);
                int newB = (int) Math.round(b * bFactor);
    ...
    public static PixelPicture alphaBlend(
        ...
        for (int rowIndex = 0; rowIndex < result.length; rowIndex ++) {
            for (int colIndex = 0; colIndex < result[0].length; colIndex ++) {
                ...
                int r2 = source2[rowIndex][colIndex].getRed();
                int g2 = source2[rowIndex][colIndex].getGreen();
                int b2 = source2[rowIndex][colIndex].getBlue();
